...
for i=1:nx
    for j=1:ny
        if ((1<=y(j))&&(y(j)<=1.1)&&(1<=x(i))&&(x(i)<=1.1))
            u(i,j)=2;
        else
            u(i,j)=0;
        end
...
bc2(1,:)=-UnW/dx; bc2(nx-2,:)=UnE/dx;  %Neumann B.Cs
bc2(:,1)=-UnS/dy; bc2(:,nx-2)=UnN/dy;  %Neumann B.Cs
%B.Cs at the corners:
bc(1,1)=UW/dx^2+US/dy^2; bc(nx-2,1)=UE/dx^2+US/dy^2;
bc(1,ny-2)=UW/dx^2+UN/dy^2; bc(nx-2,ny-2)=UE/dx^2+UN/dy^2;
bc=vis*dt*bc;

bc2(1,1)=UW/dx^2+US/dy^2; bc2(nx-2,1)=UE/dx^2+US/dy^2;
bc2(1,ny-2)=UW/dx^2+UN/dy^2; bc2(nx-2,ny-2)=UE/dx^2+UN/dy^2;
bc2=vis2*dt*bc2;
